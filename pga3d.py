...
class PGA3D:
    def __init__(self, value: float = 0.0, index: int = 0) -> None:
        ...
        :param index: Integer
        """
        self.mvec: List[float] = [0.0] * 16
        self._base: List[str] = [
            "1", "e0", "e1", "e2", "e3",  # scalar, vectors
            "e01", "e02", "e03", "e12", "e31", "e23",  # bivectors
            "e021", "e013", "e032", "e123", "e0123"  # pseudo-scalars
        ]
        if value != 0.0:
    ...
    def from_array(cls, array):
        ...
        if len(array) != len(self):
            raise TypeError("length of array must be identical to the dimension of the algebra")
        self.mvec = array
        return self
        ...
        if isinstance(self.mvec, list):
            res = " + ".join(
                filter(None, [
                    ...
                    + (["", self._base[i]][i > 0]) if abs(x) > 0.000001
                    else None
    ...
    def smul(a, b):
        if type(a) in (int, float) and isinstance(b, PGA3D):
            res = b.mvec.copy()
            res[0] = a*b[0]
            res[1] = a*b[1]
            res[2] = a*b[2]
            ...
            raise TypeError(f"PGA3D.smul: unsupported operand type(s) for *: {type(a)} and {type(b)}")
